MoveDescriptions::
; entries correspond to move ids (see constants/move_constants.asm)
	indirect_table 2, 1
	indirect_entries NUM_ATTACKS, MoveDescriptions1
	indirect_table_end

MoveDescriptions1:
	dw PoundDescription
	dw KarateChopDescription
	dw DoubleslapDescription
	dw CometPunchDescription
	dw MegaPunchDescription
	dw PayDayDescription
	dw FirePunchDescription
	dw IcePunchDescription
	dw ThunderpunchDescription
	dw ScratchDescription
	dw VicegripDescription
	dw GuillotineDescription
	dw KarateChopDescription ; Razor Wind
	dw SwordsDanceDescription
	dw ScratchDescription ; Cut
	dw GustDescription
	dw WingAttackDescription
	dw WhirlwindDescription
	dw FlyDescription
	dw BindDescription
	dw SlamDescription
	dw VineWhipDescription
	dw BiteDescription ; Stomp
	dw DoubleKickDescription
	dw MegaKickDescription
	dw JumpKickDescription
	dw RollingKickDescription
	dw SandAttackDescription
	dw HeadbuttDescription
	dw HornAttackDescription
	dw FuryAttackDescription
	dw HornDrillDescription
	dw TackleDescription
	dw BodySlamDescription
	dw WrapDescription
	dw TakeDownDescription
	dw ThrashDescription
	dw TakeDownDescription ; Double Edge
	dw TailWhipDescription
	dw PoisonStingDescription
	dw TwineedleDescription
	dw PinMissileDescription
	dw LeerDescription
	dw BiteDescription
	dw GrowlDescription
	dw RoarDescription
	dw SleepPowderDescription ; Sing
	dw SupersonicDescription
	dw SonicboomDescription
	dw DisableDescription
	dw AcidDescription
	dw EmberDescription
	dw EmberDescription ; Flamethrower
	dw MistDescription
	dw WaterGunDescription
	dw HydroPumpDescription
	dw SurfDescription
	dw IceBeamDescription
	dw IceBeamDescription ; Blizzard
	dw PsybeamDescription
	dw BubblebeamDescription
	dw AuroraBeamDescription
	dw HyperBeamDescription
	dw PeckDescription
	dw DrillPeckDescription
	dw SubmissionDescription
	dw LowKickDescription
	dw CounterDescription
	dw SeismicTossDescription
	dw StrengthDescription
	dw AbsorbDescription
	dw AbsorbDescription ; Mega Drain
	dw LeechSeedDescription
	dw GrowthDescription
	dw KarateChopDescription ; Razor Leaf
	dw SolarbeamDescription
	dw PoisonpowderDescription
	dw StunSporeDescription
	dw SleepPowderDescription
	dw ThrashDescription ; Petal Dance
	dw StringShotDescription
	dw DragonRageDescription
	dw FireSpinDescription
	dw BodySlamDescription ; Thundershock
	dw BodySlamDescription ; Thunderbolt
	dw ThunderWaveDescription
	dw BodySlamDescription ; Thunder
	dw RockThrowDescription
	dw EarthquakeDescription
	dw FissureDescription
	dw DigDescription
	dw ToxicDescription
	dw ConfusionDescription
	dw PsychicMDescription
	dw HypnosisDescription
	dw MeditateDescription
	dw AgilityDescription
	dw QuickAttackDescription
	dw RageDescription
	dw TeleportDescription
	dw NightShadeDescription
	dw MimicDescription
	dw ScreechDescription
	dw DoubleTeamDescription
	dw RecoverDescription
	dw HardenDescription
	dw MinimizeDescription
	dw SmokescreenDescription
	dw ConfuseRayDescription
	dw WithdrawDescription
	dw DefenseCurlDescription
	dw BarrierDescription
	dw LightScreenDescription
	dw HazeDescription
	dw ReflectDescription
	dw FocusEnergyDescription
	dw RockTombDescription
	dw MetronomeDescription
	dw MirrorMoveDescription
	dw SelfdestructDescription
	dw EggBombDescription
	dw BodySlamDescription ; Lick
	dw PoisonStingDescription ; Smog
	dw PoisonStingDescription ; Sludge
	dw BiteDescription ; Bone Club
	dw FlameWheelDescription ; Fire Blast
	dw WaterfallDescription
	dw ClampDescription
	dw SwiftDescription
	dw SkullBashDescription
	dw SpikeCannonDescription
	dw ConstrictDescription
	dw AmnesiaDescription
	dw KinesisDescription
	dw SoftboiledDescription
	dw HiJumpKickDescription
	dw ThunderWaveDescription ; Glare
	dw DreamEaterDescription
	dw PoisonpowderDescription ; Poison Gas
	dw BarrageDescription
	dw AbsorbDescription ; Leech Life
	dw SleepPowderDescription ; Lovely Kiss
	dw SkyAttackDescription
	dw TransformDescription
	dw BubbleDescription
	dw DizzyPunchDescription
	dw SporeDescription
	dw FlashDescription
	dw PsywaveDescription
	dw SplashDescription
	dw AcidArmorDescription
	dw KarateChopDescription ; Crabhammer
	dw ExplosionDescription
	dw FurySwipesDescription
	dw BonemerangDescription
	dw RestDescription
	dw BiteDescription ; Rock Slide
	dw BiteDescription ; Hyper Fang
	dw SharpenDescription
	dw ConversionDescription
	dw TriAttackDescription
	dw SuperFangDescription
	dw KarateChopDescription ; Slash
	dw SubstituteDescription
	dw StruggleDescription
	dw SketchDescription
	dw TripleKickDescription
	dw ThiefDescription
	dw MeanLookDescription ; Spider Web
	dw LockOnDescription ; Mind Reader
	dw FeatherdanceDescription
	dw FlameWheelDescription
	dw SnoreDescription
	dw CurseDescription
	dw FlailDescription
	dw CalmMindDescription
	dw KarateChopDescription ; Aeroblast
	dw CottonSporeDescription
	dw ReversalDescription
	dw SpiteDescription
	dw PowderSnowDescription
	dw ProtectDescription
	dw MachPunchDescription
	dw CottonSporeDescription ; Scary Face
	dw FaintAttackDescription
	dw SweetKissDescription
	dw BellyDrumDescription
	dw PoisonStingDescription ; Sludge Bomb
	dw KinesisDescription ; Mud Slap
	dw OctazookaDescription
	dw SpikesDescription
	dw ZapCannonDescription
	dw ForesightDescription
	dw DestinyBondDescription
	dw PerishSongDescription
	dw IcyWindDescription
	dw DetectDescription
	dw BoneRushDescription
	dw LockOnDescription
	dw ThrashDescription ; Outrage
	dw SandstormDescription
	dw AbsorbDescription ; Giga Drain
	dw EndureDescription
	dw CharmDescription
	dw RolloutDescription
	dw FalseSwipeDescription
	dw SwaggerDescription
	dw RecoverDescription ; Milk Drink
	dw BodySlamDescription ; Spark
	dw FuryCutterDescription
	dw SteelWingDescription
	dw MeanLookDescription
	dw AttractDescription
	dw SleepTalkDescription
	dw HealBellDescription
	dw ReturnDescription
	dw PresentDescription
	dw CosmicPowerDescription
	dw SafeguardDescription
	dw PainSplitDescription
	dw EmberDescription ; Sacred Fire
	dw MagnitudeDescription
	dw DynamicpunchDescription
	dw MegahornDescription
	dw DragonbreathDescription
	dw BatonPassDescription
	dw EncoreDescription
	dw PursuitDescription
	dw RapidSpinDescription
	dw SweetScentDescription
	dw IronTailDescription
	dw MetalClawDescription
	dw VitalThrowDescription
	dw MorningSunDescription
	dw MorningSunDescription ; Synthesis
	dw MorningSunDescription ; Moonlight
	dw HiddenPowerDescription
	dw KarateChopDescription ; Cross Chop
	dw TwisterDescription
	dw RainDanceDescription
	dw SunnyDayDescription
	dw CrunchDescription
	dw MirrorCoatDescription
	dw PsychUpDescription
	dw ExtremespeedDescription
	dw AncientpowerDescription
	dw ShadowBallDescription
	dw FutureSightDescription
	dw RockSmashDescription
	dw ClampDescription ; Whirlpool
	dw StrengthSapDescription
	dw DarkPulseDescription
	dw DragonClawDescription
	dw MirrorShotDescription
	dw DragonPulseDescription
	dw KarateChopDescription ; Night Slash
	dw IceShardDescription
	dw FlashCannonDescription
	dw BugBuzzDescription
	dw BraveBirdDescription
	dw DrainPunchDescription
	dw SilverWindDescription
	dw KarateChopDescription ; Shadow Claw
	dw BulldozeDescription
	dw SeedBombDescription
	dw DragonRushDescription
	dw BulletPunchDescription
	dw ZenHeadbuttDescription
	dw PoisonJabDescription
	dw KarateChopDescription ; Drill Run
	dw DrainingKissDescription
	dw FairyWindDescription
	dw MetalSoundDescription
	dw DisarmVoiceDescription
	dw WildChargeDescription
	dw PowerWhipDescription
	dw DazzlinGleamDescription
	dw GigaImpactDescription
	dw NastyPlotDescription
	dw PlayRoughDescription
	dw FocusBlastDescription
	dw WaterPulseDescription
	dw GunkShotDescription
	dw SludgeWaveDescription
	dw EarthPowerDescription
	dw IronHeadDescription
	dw SignalBeamDescription
	dw IcicleCrashDescription
	dw KarateChopDescription ; Stone Edge
	dw ExtrasensoryDescription
	dw XScissorDescription
	dw AquaJetDescription
	dw AquaTailDescription
	dw AerialAceDescription
	dw MagicalLeafDescription
	dw EnergyBallDescription
	dw PowerGemDescription
	dw MudBombDescription
	dw HexDescription
	dw VenoshockDescription
	dw HurricaneDescription
	dw FireFangDescription
	dw IceFangDescription
	dw ThunderFangDescription
	dw DracoFangDescription
	dw SteelSliceDescription
	dw PixiePunchDescription
	dw WillOWispDescription
	dw MoonblastDescription
	dw FlareBlitzDescription
	dw HyperVoiceDescription
	dw AirSlashDescription
	dw AvalancheDescription
	dw AvalancheDescription ; Revenge
	dw RockWreckerDescription
	dw IcicleSpearDescription
	dw FlameBurstDescription
	dw DischargeDescription
	dw AcidArmorDescription ; Iron Defense
	dw RockBlastDescription
	dw BulletSeedDescription
	dw PetalBlizzDescription
	dw SolarbeamDescription ; Solar Blade
	dw InfernoDescription
	dw WoodHammerDescription
	dw PaybackDescription
	dw SnarlDescription
	dw AstonishDescription
	dw ShadowPunchDescription
	dw RoundMDescription
	dw HiHorsepowerDescription
	dw MudShotDescription
	dw SandTombDescription
	dw AuraSphereDescription
	dw HammerArmDescription
	dw CloseCombatDescription
	dw LowSweepDescription
	dw BulkUpDescription
	dw SuperpowerDescription
	dw PowerUpPunchDescription
	dw DragonDanceDescription
	dw ShellSmashDescription
	dw HoneClawsDescription
	dw PoisonFangDescription
	dw FlameChargeDescription
	dw HailDescription
	dw BrickBreakDescription
	dw QuiverDanceDescription
	dw RoostDescription
	dw CrossPoisonDescription
	dw SmartStrikeDescription
	dw BelchDescription
	dw AcidSprayDescription
	dw NuzzleDescription
	dw VoltTackleDescription
	dw CrushClawDescription
	dw PlayNiceDescription
	dw KarateChopDescription ; Air Cutter
	dw OdorSleuthDescription
	dw HowlDescription
	dw CircleThrowDescription
	dw KarateChopDescription ; Psycho Cut
	dw DualChopDescription
	dw BounceDescription
	dw RockPolishDescription
	dw GyroBallDescription
	dw RecoverDescription ; Slack Off
	dw DoubleHitMDescription
	dw ChargeBeamDescription
	dw BlazeKickDescription
	dw DragonTailDescription
	dw SheerColdDescription
	dw KarateChopDescription ; Leaf Blade
	dw SuckerPunchDescription
	dw LowKickDescription ; Grass Knot
	dw GrowthDescription ; Work Up
	dw FuryCutterDescription ; Echoed Voice
	dw MuddyWaterDescription
	dw HeatWaveDescription
	dw LavaPlumeDescription
	dw StoneBashDescription
	dw QuickAttackDescription ; Shadow Sneak
	dw BiteDescription ; Needle Arm
	dw FlatterDescription
	dw SleepPowderDescription ; Grass Whistle
	dw AncientpowerDescription
	dw HyperBeamDescription ; Frenzy Plant
	dw HyperBeamDescription ; Blast Burn
	dw HyperBeamDescription ; Hydro Cannon
	dw AquaRingDescription
	dw KnockOffDescription
	dw UTurnDescription
	dw AcrobaticsDescription
	dw UTurnDescription ; Volt Switch
	dw CoilDescription
	dw PincirFlurryDescription
	dw BubblebeamDescription
	dw SignalWaveDescription
	dw TakeDownDescription ; Wave Crash
	dw FakeOutDescription
	dw CloseCombatDescription
	dw DoubleHitMDescription
	dw DoubleHitMDescription
	dw CatastropheDescription
	dw PsyshieldDescription
	dw MeteorMashDescription
	dw ForcePalmDescription
	dw SkyUppercutDescription
	dw PsychoBoostDescription
	dw HeadSmashDescription
	dw ThrashDescription ; Raging Fury
	dw StrangeSteamDescription
	dw ShadowBoneDescription
	dw CrossPoisonDescription ; Poison Tail
	dw FreezeGlareDescription
	dw ThunderKickDescription
	dw FieryWrathDescription
	dw ShellSideArmDescription
	dw EerieSpellDescription
;	dw WakeUpSlapDescription

InvalidMoveDescription:
	db "?@"

PoundDescription:
	db   "Pounds with fore-"
	line "legs or tail.@"

KarateChopDescription:
	db   "Has a high criti-"
	line "cal hit ratio.@"

DoubleslapDescription:
	db   "Repeatedly slaps"
	line "2-5 times.@"

CometPunchDescription:
	db   "Repeatedly punches"
	line "2-5 times.@"

MegaPunchDescription:
	db   "A powerful punch"
	line "thrown very hard.@"

PayDayDescription:
	db   "Throws coins. Gets"
	line "them back later.@"

FirePunchDescription:
	db   "A fiery punch. May"
	line "cause a burn.@"

IcePunchDescription:
	db   "An icy punch. May"
	line "cause freezing.@"

ThunderpunchDescription:
	db   "An electric punch."
	line "It may paralyze.@"

ScratchDescription:
	db   "Scratches with"
	line "sharp claws.@"

VicegripDescription:
	db   "Grips with power-"
	line "ful pincers.@"

GuillotineDescription:
	db   "A one-hit KO,"
	line "pincer attack.@"

SwordsDanceDescription:
	db   "A dance that in-"
	line "creases Attack.@"

GustDescription:
	db   "Whips up a strong"
	line "gust of wind.@"

WingAttackDescription:
	db   "Strikes the target"
	line "with wings.@"

WhirlwindDescription:
	db   "Blows away the foe"
	line "& ends battle.@"

FlyDescription:
	db   "1st turn: Fly"
	line "2nd turn: Attack@"

BindDescription:
	db   "Binds the target"
	line "for 2-5 turns.@"

SlamDescription:
	db   "Slams the foe with"
	line "a tail, vine, etc.@"

VineWhipDescription:
	db   "Whips the foe with"
	line "slender vines.@"

DoubleKickDescription:
	db   "A double kicking"
	line "attack.@"

MegaKickDescription:
	db   "A powerful kicking"
	line "attack.@"

JumpKickDescription:
	db   "May miss, damaging"
	line "the user.@"

RollingKickDescription:
	db   "A fast, spinning"
	line "kick.@"

SandAttackDescription:
	db   "Reduces accuracy"
	line "by throwing sand.@"

HeadbuttDescription:
	db   "An attack that may"
	line "make foe flinch.@"

HornAttackDescription:
	db   "An attack using a"
	line "horn to jab.@"

FuryAttackDescription:
	db   "Jabs the target"
	line "2-5 times.@"

HornDrillDescription:
	db   "A one-hit KO,"
	line "drill attack.@"

TackleDescription:
	db   "A full-body charge"
	line "attack.@"

BodySlamDescription:
	db   "An attack that may"
	line "cause paralysis.@"

WrapDescription:
	db   "Squeezes the foe"
	line "for 2-5 turns.@"

TakeDownDescription:
	db   "A tackle that also"
	line "hurts the user.@"

ThrashDescription:
	db   "Works 2-3 turns"
	line "and confuses user.@"

TailWhipDescription:
	db   "Lowers the foe's"
	line "Defense.@"

PoisonStingDescription:
	db   "An attack that may"
	line "poison the target.@"

TwineedleDescription:
	db   "Jabs the foe twice"
	line "using stingers.@"

PinMissileDescription:
	db   "Fires pins that"
	line "strike 2-5 times.@"

LeerDescription:
	db   "Reduces the foe's"
	line "Defense.@"

BiteDescription:
	db   "An attack that may"
	line "cause flinching.@"

GrowlDescription:
	db   "Reduces the foe's"
	line "Attack.@"

RoarDescription:
	db   "Scares wild foes"
	line "to end battle.@"

SupersonicDescription:
	db   "Sound waves that"
	line "cause confusion.@"

SonicboomDescription:
	db   "Always inflicts"
	line "20HP damage.@"

DisableDescription:
	db   "Disables the foe's"
	line "most recent move.@"

AcidDescription:
	db   "An attack that may"
	line "lower Spcl.Def.@"

EmberDescription:
	db   "An attack that may"
	line "inflict a burn.@"

MistDescription:
	db   "Prevents stat"
	line "reduction.@"

WaterGunDescription:
	db   "Squirts water to"
	line "attack.@"

HydroPumpDescription:
	db   "A powerful water-"
	line "type attack.@"

SurfDescription:
	db   "A strong water-"
	line "type attack.@"

IceBeamDescription:
	db   "An attack that may"
	line "freeze the foe.@"

PsybeamDescription:
	db   "An attack that may"
	line "confuse the foe.@"

BubblebeamDescription:
	db   "An attack that may"
	line "lower Speed.@"

AuroraBeamDescription:
	db   "An attack that may"
	line "lower Attack.@"

HyperBeamDescription:
	db   "1st turn: Attack"
	line "2nd turn: Rest@"

PeckDescription:
	db   "Jabs the foe with"
	line "a beak, etc.@"

DrillPeckDescription:
	db   "A strong, spin-"
	line "ning-peck attack.@"

SubmissionDescription:
	db   "An attack that al-"
	line "so hurts the user.@"

LowKickDescription:
	db   "Damage depends on"
	line "target's weight.@"

CounterDescription:
	db   "Returns a physical"
	line "blow double.@"

SeismicTossDescription:
	db   "The user's level"
	line "equals damage HP.@"

StrengthDescription:
	db   "A powerful physi-"
	line "cal attack.@"

AbsorbDescription:
	db   "Steals 1/2 of the"
	line "damage inflicted.@"

LeechSeedDescription:
	db   "Steals HP from the"
	line "foe on every turn.@"

GrowthDescription:
	db   "Raises Attack and"
	line "Spcl.Atk.@"

SolarbeamDescription:
	db   "1st turn: Prepare"
	line "2nd turn: Attack@"

PoisonpowderDescription:
	db   "A move that may"
	line "poison the foe.@"

StunSporeDescription:
	db   "A move that may"
	line "paralyze the foe.@"

SleepPowderDescription:
	db   "May cause the foe"
	line "to fall asleep.@"

StringShotDescription:
	db   "A move that lowers"
	line "the foe's Speed.@"

DragonRageDescription:
	db   "User attacks with"
	line "full rage.@"

FireSpinDescription:
	db   "Traps foe in fire"
	line "for 2-5 turns.@"

ThunderWaveDescription:
	db   "A move that may"
	line "cause paralysis.@"

RockThrowDescription:
	db   "Drops rocks on the"
	line "enemy.@"

EarthquakeDescription:
	db   "Tough but useless"
	line "vs. flying foes.@"

FissureDescription:
	db   "A ground-type,"
	line "one-hit KO attack.@"

DigDescription:
	db   "1st turn: Burrow"
	line "2nd turn: Attack@"

ToxicDescription:
	db   "A poison move with"
	line "increasing damage.@"

ConfusionDescription:
	db   "An attack that may"
	line "cause confusion.@"

PsychicMDescription:
	db   "An attack that may"
	line "lower Spcl.Def.@"

HypnosisDescription:
	db   "May put the foe to"
	line "sleep.@"

MeditateDescription:
	db   "Raises the user's"
	line "Attack.@"

AgilityDescription:
	db   "Sharply increases"
	line "the user's Speed.@"

QuickAttackDescription:
	db   "Lets the user get"
	line "in the first hit.@"

RageDescription:
	db   "Raises Attack if"
	line "the user is hit.@"

TeleportDescription:
	db   "A move for fleeing"
	line "from battle.@"

NightShadeDescription:
	db   "The user's level"
	line "equals damage HP.@"

MimicDescription:
	db   "Copies a move used"
	line "by the foe.@"

ScreechDescription:
	db   "Harshly reduces"
	line "the foe's Defense.@"

DoubleTeamDescription:
	db   "Heightens evasive-"
	line "ness.@"

RecoverDescription:
	db   "Restores HP by 1/2"
	line "the max HP.@"

HardenDescription:
	db   "Raises the user's"
	line "Defense.@"

MinimizeDescription:
	db   "Heightens evasive-"
	line "ness.@"

SmokescreenDescription:
	db   "Lowers the foe's"
	line "accuracy.@"

ConfuseRayDescription:
	db   "A move that causes"
	line "confusion.@"

WithdrawDescription:
	db   "Heightens the"
	line "user's Defense.@"

DefenseCurlDescription:
	db   "Heightens the"
	line "user's Defense.@"

BarrierDescription:
	db   "Sharply increases"
	line "user's Defense.@"

LightScreenDescription:
	db   "Ups Spcl.Def with"
	line "a wall of light.@"

HazeDescription:
	db   "Eliminates all"
	line "stat changes.@"

ReflectDescription:
	db   "Raises Defense"
	line "with a barrier.@"

FocusEnergyDescription:
	db   "Raises the criti-"
	line "cal hit ratio.@"

RockTombDescription:
	db	 "Lowers Speed by"
	line "piling rocks.@"

MetronomeDescription:
	db   "Randomly uses any"
	line "#mon move.@"

MirrorMoveDescription:
	db   "Counters with the"
	line "same move.@"

SelfdestructDescription:
	db   "Powerful but makes"
	line "the user faint.@"

EggBombDescription:
	db   "Eggs are hurled at"
	line "the foe.@"

WaterfallDescription:
	db   "An aquatic charge"
	line "attack.@"

ClampDescription:
	db   "Traps the foe for"
	line "2-5 turns.@"

SwiftDescription:
	db   "An attack that"
	line "never misses.@"

SkullBashDescription:
	db   "1st turn: Prepare"
	line "2nd turn: Attack@"

SpikeCannonDescription:
	db   "Fires spikes to"
	line "hit 2-5 times.@"

ConstrictDescription:
	db   "An attack that may"
	line "lower SPEED.@"

AmnesiaDescription:
	db   "Sharply raises the"
	line "user's Spcl.Def.@"

KinesisDescription:
	db   "Reduces the foe's"
	line "accuracy.@"

SoftboiledDescription:
	db   "Restores HP by 1/2"
	line "the user's max HP.@"

HiJumpKickDescription:
	db   "May miss and hurt"
	line "the user.@"

DreamEaterDescription:
	db   "Steals HP from a"
	line "sleeping victim.@"

BarrageDescription:
	db   "Throws orbs to hit"
	line "2-5 times.@"

SkyAttackDescription:
	db   "1st turn: Prepare"
	line "2nd turn: Attack@"

TransformDescription:
	db   "The user assumes"
	line "the foe's guise.@"

BubbleDescription:
	db   "An attack that may"
	line "reduce Speed.@"

DizzyPunchDescription:
	db   "An attack that may"
	line "cause confusion.@"

SporeDescription:
	db   "A move that"
	line "induces sleep.@"

FlashDescription:
	db   "Blinds the foe to"
	line "reduce accuracy.@"

PsywaveDescription:
	db   "An attack with"
	line "variable power.@"

SplashDescription:
	db   "Has no effect"
	line "whatsoever.@"

AcidArmorDescription:
	db   "Sharply raises the"
	line "user's Defense.@"

ExplosionDescription:
	db   "Very powerful but"
	line "makes user faint.@"

FurySwipesDescription:
	db   "Quickly scratches"
	line "2-5 times.@"

BonemerangDescription:
	db   "An attack that"
	line "strikes twice.@"

RestDescription:
	db   "Sleep for 2 turns"
	line "to fully recover.@"

SharpenDescription:
	db   "A move that raises"
	line "the user's Attack.@"

ConversionDescription:
	db   "Change user's type"
	line "to a move's type.@"

TriAttackDescription:
	db   "Fires three kinds"
	line "of beams at once.@"

SuperFangDescription:
	db   "Cuts the foe's HP"
	line "by 1/2.@"

SubstituteDescription:
	db   "Makes a decoy with"
	line "1/4 user's max HP.@"

StruggleDescription:
	db   "Used only if all"
	line "PP are exhausted.@"

SketchDescription:
	db   "Copies the foe's"
	line "move permanently.@"

TripleKickDescription:
	db   "Hits three times"
	line "with rising power.@"

ThiefDescription:
	db   "An attack that may"
	line "steal a held item.@"

FeatherdanceDescription:
	db	 "Harshly lowers Atk"
	line "with feathers.@"

FlameWheelDescription:
	db   "An attack that may"
	line "cause a burn.@"

SnoreDescription:
	db   "An attack useable"
	line "only while asleep.@"

CurseDescription:
	db   "Works differently"
	line "for ghost-types.@"

FlailDescription:
	db   "Stronger if the"
	line "user's HP is low.@"

CalmMindDescription:
	db   "Raises Spcl.Atk"
	line "and Spcl.Def.@"

CottonSporeDescription:
	db   "Harshly reduces"
	line "the foe's Speed.@"

ReversalDescription:
	db   "Stronger if the"
	line "user's HP is low.@"

SpiteDescription:
	db   "Cuts the PP of the"
	line "foe's last move.@"

PowderSnowDescription:
	db   "An attack that may"
	line "cause freezing.@"

ProtectDescription:
	db   "Foils attack that"
	line "turn. It may fail.@"

MachPunchDescription:
	db   "A fast punch that"
	line "lands first.@"

FaintAttackDescription:
	db   "An attack that"
	line "never misses.@"

SweetKissDescription:
	db   "A move that causes"
	line "confusion.@"

BellyDrumDescription:
	db   "Reduces own HP to"
	line "maximize Attack.@"

OctazookaDescription:
	db   "An attack that may"
	line "reduce accuracy.@"

SpikesDescription:
	db   "Hurts foes when"
	line "they switch out.@"

ZapCannonDescription:
	db   "An attack that"
	line "always paralyzes.@"

ForesightDescription:
	db   "Negates accuracy"
	line "reduction moves.@"

DestinyBondDescription:
	db   "The foe faints if"
	line "the user does.@"

PerishSongDescription:
	db   "Both user and foe"
	line "faint in 3 turns.@"

IcyWindDescription:
	db   "An icy attack that"
	line "lowers Speed.@"

DetectDescription:
	db   "Evades attack that"
	line "turn. It may fail.@"

BoneRushDescription:
	db   "An attack that"
	line "hits 2-5 times.@"

LockOnDescription:
	db   "Ensures the next"
	line "attack will hit.@"

SandstormDescription:
	db   "Inflicts damage"
	line "every turn.@"

EndureDescription:
	db   "Always leaves at"
	line "least 1HP.@"

CharmDescription:
	db   "Harshly lowers the"
	line "foe's Attack.@"

RolloutDescription:
	db   "Attacks 5 turns"
	line "with rising power.@"

FalseSwipeDescription:
	db   "Leaves the foe"
	line "with at least 1HP.@"

SwaggerDescription:
	db   "Causes confusion"
	line "and raises Attack.@"

FuryCutterDescription:
	db   "Successive hits"
	line "raise power.@"

SteelWingDescription:
	db   "Stiff wings strike"
	line "the foe.@"

MeanLookDescription:
	db   "Prevents fleeing"
	line "or switching.@"

AttractDescription:
	db   "Makes the opposite"
	line "gender infatuated.@"

SleepTalkDescription:
	db   "Randomly attacks"
	line "while asleep.@"

HealBellDescription:
	db   "Eliminates all"
	line "status problems.@"

ReturnDescription:
	db   "An attack that is"
	line "based on loyalty.@"

PresentDescription:
	db   "A bomb that may"
	line "restore HP.@"

CosmicPowerDescription:
	db	 "Raises Defense and"
	line "Spcl.Def.@"

SafeguardDescription:
	db   "Prevents all"
	line "status problems.@"

PainSplitDescription:
	db   "Adds user & foe's"
	line "HPs. Shares total.@"

MagnitudeDescription:
	db   "A ground attack"
	line "with random power.@"

DynamicpunchDescription:
	db   "An attack that"
	line "always confuses.@"

MegahornDescription:
	db   "A powerful charge"
	line "attack.@"

DragonbreathDescription:
	db   "A strong breath"
	line "attack.@"

BatonPassDescription:
	db   "Switches while"
	line "keeping effects.@"

EncoreDescription:
	db   "Makes the foe re-"
	line "peat 2-6 times.@"

PursuitDescription:
	db   "Heavily strikes"
	line "switching #mon.@"

RapidSpinDescription:
	db   "A high-speed"
	line "spinning attack.@"

SweetScentDescription:
	db   "Reduces the foe's"
	line "evasiveness.@"

IronTailDescription:
	db   "An attack that may"
	line "reduce Defense.@"

MetalClawDescription:
	db   "An attack that may"
	line "up user's Attack@"

VitalThrowDescription:
	db   "A 2nd-strike move"
	line "that never misses.@"

MorningSunDescription:
	db   "Restores HP"
	line "(varies by time).@"

HiddenPowerDescription:
	db   "The power varies"
	line "with the #mon.@"

TwisterDescription:
	db   "Whips up a tornado"
	line "to attack.@"

RainDanceDescription:
	db   "Boosts water-type"
	line "moves for 5 turns.@"

SunnyDayDescription:
	db   "Boosts fire-type"
	line "moves for 5 turns.@"

CrunchDescription:
	db   "An attack that may"
	line "lower Defense.@"

MirrorCoatDescription:
	db   "Counters a Spcl."
	line "Atk move double.@"

PsychUpDescription:
	db   "Copies the foe's"
	line "stat changes.@"

ExtremespeedDescription:
	db   "A powerful first-"
	line "strike move.@"

AncientpowerDescription:
	db   "An attack that may"
	line "raise all stats.@"

ShadowBallDescription:
	db   "An attack that may"
	line "lower Spcl.Def.@"

FutureSightDescription:
	db   "An attack that"
	line "hits on 3rd turn.@"

RockSmashDescription:
	db   "An attack that may"
	line "lower Defense.@"

StrengthSapDescription:
	db   "Heals user and lo-"
	line "wers foe's Attack.@"

DarkPulseDescription:
	db   "Unleash dark ener-"
	line "gy, causes flinch.@"

DragonClawDescription:
	db   "Slashes the foe"
	line "with sharp claws.@"

MirrorShotDescription:
	db   "Flash of energy."
	line "Lowers accuracy.@"

DragonPulseDescription:
	db   "Blasts foe with"
	line "powerful wave.@"

IceShardDescription:
	db   "Strikes first with"
	line "sharp icicles.@"

FlashCannonDescription:
	db   "Cannon of light."
	line "Lowers Spcl.Def.@"

BugBuzzDescription:
	db   "Obnoxious buzzing,"
	line "lowers Spcl.Def.@"

BraveBirdDescription:
	db   "Flying tackle that"
	line "hurts the user.@"

DrainPunchDescription:
	db   "Punch that heals"
	line "the user.@"

SilverWindDescription:
	db   "Powdery attack, c-"
	line "an increase stats.@"

BulldozeDescription:
	db   "Runs foe over and"
	line "lowers its Speed.@"

SeedBombDescription:
	db   "Hard-shelled seeds"
	line "barrage the foe.@"

DragonRushDescription:
	db   "Powerful tackle."
	line "May cause finch.@"

BulletPunchDescription:
	db   "Fist of steel that"
	line "strikes first.@"

ZenHeadbuttDescription:
	db   "Inner peace bash,"
	line "can cause flinch.@"

PoisonJabDescription:
	db   "Powerful jab that"
	line "can poison foe.@"

DrainingKissDescription:
	db   "Sweet kiss that"
	line "heals the user.@"

FairyWindDescription:
	db   "User stirs up a"
	line "fairy wind.@"

MetalSoundDescription:
	db   "Harsh sound that"
	line "lowers Splc.Def.@"

DisarmVoiceDescription:
	db   "Shrill shriek that"
	line "never misses.@"

WildChargeDescription:
	db   "Electric tackle,"
	line "hurts the user.@"

PowerWhipDescription:
	db   "Powerful vine"
	line "attack.@"

DazzlinGleamDescription:
	db   "User emits a powe-"
	line "rful flash.@"

GigaImpactDescription:
	db   "1st turn: Attack"
	line "2nd turn: Rest@"

NastyPlotDescription:
	db   "Sharply raises the"
	line "user's Spcl.Atk.@"

PlayRoughDescription:
	db   "Plays without mer-"
	line "cy. Can lower Atk.@"

FocusBlastDescription:
	db   "Spcl.Def can be"
	line "lowered.@"

WaterPulseDescription:
	db   "Can confuse with"
	line "pulse of water.@"

GunkShotDescription:
	db   "Filthy garbage th-"
	line "at can poison.@"

SludgeWaveDescription:
	db	 "Swamps the foe's"
	line "area with poison.@"

EarthPowerDescription:
	db   "Ground erupts, can"
	line "lower Spcl.Def.@"

IronHeadDescription:
	db   "Can cause flinch"
	line "with a hard head.@"

SignalBeamDescription:
	db   "Beam that can can"
	line "cause confusion.@"

IcicleCrashDescription:
	db   "Causes flinching"
	line "with big icicles.@"

ExtrasensoryDescription:
	db   "May cause flinchi-"
	line "ng with the mind.@"

XScissorDescription:
	db   "Slices foe with"
	line "crossed limbs.@"

AquaJetDescription:
	db   "Strikes first with"
	line "jet propulsion.@"

AquaTailDescription:
	db   "A watery tail hits"
	line "the foe.@"

AerialAceDescription:
	db   "Aerial strike that"
	line "never misses.@"

MagicalLeafDescription:
	db   "Mystical leaves"
	line "that never miss.@"

EnergyBallDescription:
	db   "Sphere of energy,"
	line "lowers Spcl.Def.@"

PowerGemDescription:
	db   "Attacks foe with"
	line "gemstones.@"

MudBombDescription:
	db   "Mud ball that can"
	line "lower accuracy.@"

HexDescription:
	db   "2x damage on foe"
	line "with status.@"

VenoshockDescription:
	db   "Deals 2x damage on"
	line "poisoned foe.@"

HurricaneDescription:
	db   "May confuse, does-"
	line "n't miss in rain.@"

FireFangDescription:
	db   "Searing teeth."
	line "Flinch or burn.@"

IceFangDescription:
	db   "Really cold jaws."
	line "Flinch or freeze.@"

ThunderFangDescription:
	db   "Stunning jaws, fl-"
	line "inch or paralyze.@"

DracoFangDescription:
	db   "Deadly wyrm fangs."
	line "Flinch or confuse.@"

SteelSliceDescription:
	db   "Can lower all"
	line "foe's stats.@"

PixiePunchDescription:
	db   "Enchanted punch,"
	line "can make sleep.@"

WillOWispDescription:
	db   "Burns foe with"
	line "ghostly fire.@"

MoonblastDescription:
	db   "May lower Spcl.Atk"
	line "with moon power.@"

FlareBlitzDescription:
	db   "Firey tackle that"
	line "hurts the user.@"

HyperVoiceDescription:
	db   "Loud and obnoxious"
	line "voice.@"

AirSlashDescription:
	db   "Slice of air that"
	line "causes flinching.@"

AvalancheDescription:
	db   "Go 2nd. Power rai-"
	line "sed after damage.@"

RockWreckerDescription:
	db	 "Launches huge rock"
	line "but must recharge.@"

IcicleSpearDescription:
	db   "Fires ice at the"
	line "foe 2-5 times.@"

FlameBurstDescription:
	db	 "Attacks foe with"
	line "burst of fire.@"

DischargeDescription:
	db	 "Flare of electric-"
	line "ity, can paralyze.@"

RockBlastDescription:
	db	 "Hurls rocks at foe"
	line "2-5 times.@"

BulletSeedDescription:
	db	 "Shoots 2-5 seeds"
	line "at the foe@"

PetalBlizzDescription:
	db	 "Stirs up violent"
	line "petal blizzard.@"

InfernoDescription:
	db	 "Foe is engulfed in"
	line "intense fire.@"

WoodHammerDescription:
	db	 "Slams its body"
	line "into foe. Recoil.@"

PaybackDescription:
	db	 "Power is doubled"
	line "after damaged.@"

SnarlDescription:
	db	 "User yells and"
	line "lowers Spcl.Atk.@"

AstonishDescription:
	db	 "May shock foe into"
	line "flinching.@"

ShadowPunchDescription:
	db	 "Punch from the sh-"
	line "adows, never miss.@"

RoundMDescription:
	db	 "Attacks with a"
	line "dangerous song.@"

HiHorsepowerDescription:
	db	 "Fiercely attacks"
	line "with full body.@"

MudShotDescription:
	db	 "Hurls mud and"
	line "lowers Speed.@"

SandTombDescription:
	db	 "Traps foe in sand"
	line "for 2-5 turns.@"

AuraSphereDescription:
	db	 "Blast of aura that"
	line "always hits.@"

HammerArmDescription:
	db	 "Heavy hit that lo-"
	line "wers user's Speed.@"

CloseCombatDescription:
	db   "Lowers user's Def"
	line "and Spcl.Def.@"

LowSweepDescription:
	db	 "Sweeping kick that"
	line "lowers Speed.@"

BulkUpDescription:
	db	 "Boosts Attack and"
	line "Defense.@"

SuperpowerDescription:
	db	 "Lowers user's Att-"
	line "ack and Defense.@"

PowerUpPunchDescription:
	db	 "Will always raise"
	line "user's Attack.@"

DragonDanceDescription:
	db   "Raises Atk and"
	line "Speed with dance.@"

ShellSmashDescription:
	db   "Drops def. stats."
	line "Raises others.@"

HoneClawsDescription:
	db   "Raise Attack and"
	line "Accuracy.@"

PoisonFangDescription:
	db	 "Fangs may badly"
	line "poison the foe.@"

FlameChargeDescription:
	db	 "Raises user's"
	line "Speed with flames.@"

HailDescription:
	db	 "Summons hail for"
	line "5 turns.@"

BrickBreakDescription:
	db	 "Breaks screens"
	line "with fists.@"

QuiverDanceDescription:
	db	 "Raise Spcl.Atk, S-"
	line "pcl.Def and Speed.@"

RoostDescription:
	db	 "Users rests its"
	line "wings and heals.@"

CrossPoisonDescription:
	db	 "High crit. rate,"
	line "can poison.@"

SmartStrikeDescription:
	db	 "Never misses with"
	line "with a sharp horn.@"

BelchDescription:
	db	 "A horrible belch"
	line "that can poison.@"

AcidSprayDescription:
	db	 "Acid that harshly"
	line "lowers Spcl.Def.@"

NuzzleDescription:
	db	 "Nuzzles foe to"
	line "always paralyze.@"

VoltTackleDescription:
	db	 "Dangerous tackle"
	line "that hurts user.@"

CrushClawDescription:
	db	 "Tears at foe, may"
	line "lower Defense.@"

PlayNiceDescription:
	db	 "Lowers Attack and"
	line "never misses.@"

OdorSleuthDescription:
	db   "Negates accuracy"
	line "reduction moves.@"

HowlDescription:
	db	 "Howls to raise"
	line "Attack stat.@"

CircleThrowDescription:
	db	 "Goes last and"
	line "switches target.@"

DualChopDescription:
	db	 "Brutally chops the"
	line "foe twice.@"

BounceDescription:
	db   "Bouncing attack,"
	line "may paralyze.@"

RockPolishDescription:
	db	 "Polishes body to"
	line "increase Speed.@"

GyroBallDescription:
	db   "User spins and"
	line "deals damage.@"

DoubleHitMDescription:
	db	 "Hits the foe"
	line "twice.@"

ChargeBeamDescription:
	db	 "Most likely will"
	line "raise Spcl.Atk.@"

BlazeKickDescription:
	db	 "Hi-crit ratio, may"
	line "leave a burn.@"

DragonTailDescription:
	db	 "Knocks away the"
	line "foe.@"

SheerColdDescription:
	db   "A one-hit KO,"
	line "freeze attack.@"

SuckerPunchDescription:
	db	 "Won't work if foe"
	line "doesn't attack.@"

MuddyWaterDescription:
	db   "Can lower accuracy"
	line "with muddy water.@"

HeatWaveDescription:
	db   "Hot breath can"
	line "burn target.@"

LavaPlumeDescription:
	db   "Lava erupts and"
	line "may burn foe.@"

StoneBashDescription:
	db   "Hits foe with a"
	line "hardened body.@"

FlatterDescription:
	db   "Causes confusion"
	line "and raises Sp.Atk.@"

;WakeUpSlapDescription:
;	db   "Wakes up sleeping"
;	line "foe, more damage.@"

AquaRingDescription:
	db	 "Restores HP each"
	line "turn.@"

KnockOffDescription:
	db	 "Can knock off a"
	line "foe's item.@"

UTurnDescription:
	db   "After attacking,"
	line "user switches out.@"

AcrobaticsDescription:
	db   "More power when"
	line "user has no item.@"

CoilDescription:
	db   "Raises ATK, DEF"
	line "and Accuracy.@"

PincirFlurryDescription:
	db   "A barrage of"
	line "angry pincirs.@"

SignalWaveDescription:
	db   "Signals that can"
	line "cause confusion.@"

FakeOutDescription:
	db   "Foe flinches if"
	line "used first turn.@"

CatastropheDescription:
	db   "A disaster that"
	line "confuses the foe.@"

PsyshieldDescription:
	db   "A barrier attack"
	line "that raises Def.@"

MeteorMashDescription:
	db   "Meteor-like punch."
	line "May raise Attack.@"

ForcePalmDescription:
	db   "A shock wave that"
	line "can paralyze.@"

SkyUppercutDescription:
	db   "An uppercut thrown"
	line "to the sky.@"

PsychoBoostDescription:
	db   "Harshly lowers the"
	line "user's Sp.Atk.@"

HeadSmashDescription:
	db   "A reckless head-"
	line "butt with recoil.@"

StrangeSteamDescription:
	db   "Steam that may"
	line "confuse the foe.@"

ShadowBoneDescription:
	db   "Ethereal bone that"
	line "may lower Defense.@"

FreezeGlareDescription:
	db   "Icy glare that can"
	line "freeze.@"

ThunderKickDescription:
	db   "Shocking kick that"
	line "lowers Defense.@"

FieryWrathDescription:
	db   "Wrathful flames"
	line "that can flinch.@"

ShellSideArmDescription:
	db   "Quick draw poison"
	line "shot.@"

EerieSpellDescription:
	db   "Odd incantation"
	line "that lowers PP.@"
